...
Specify if the forward is specified iwth IPv6 or IPv4Optional. Type boolean.

=head2 C<bind_address>
...
The bind_address of “localhost” indicates that the listening port be bound for local use only, while an empty address or ‘*’ indicates that the port should be available from all interfaces.Optional. Type uniline.
